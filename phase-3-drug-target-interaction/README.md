...
The core idea is that a drug–target pair can be represented as a concatenation of two feature vectors, and binding becomes a binary classification problem over that joint representation.
...
- **Baseline models** — drug fingerprints plus basic QSAR descriptors only.
- **Enhanced models** — the above, plus protein sequence and AlphaFold-derived structure embeddings.

The enhanced/baseline split is the experiment: does structural information about the target improve binding prediction over chemistry alone?
...
**Read the MCC column, not the accuracy column.** The dummy classifier — which predicts the majority class for everything — reaches 68% accuracy and 0.81 F1. Random Forest's 76% is an 8-point lift on a badly imbalanced problem. Matthews correlation coefficient accounts for that imbalance, and 0.41 is a moderate signal, not a strong one.
...
Several serialised datasets exceed 50 MB and are tracked with **Git LFS**:
...
Install LFS before cloning, or these arrive as short text pointer files:
...
- **Only classification models are included in the app.** The regression pipeline (binding affinity as a continuous value) has datasets and metrics committed, but the trained regression models and their Streamlit page are not in this repository.
